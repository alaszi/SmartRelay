...
- [x] `ci.yml` green on `main`; `deploy.yml` is manual-only; no secrets in the repo history. Two real bugs found: `ci.yml` was timing out on every recent run (missing Redis service, fixed), then a green badge with a real "exit code 1" annotation (`pnpm audit` correctly catching 2 high-severity `nodemailer` CVEs behind `continue-on-error`, fixed by upgrading nodemailer). Confirmed clean by reading the Annotations panel, not just the badge, on a real run (commit `fd7a77f`, [run 35963047330](https://github.com/alaszi/SmartRelay/actions/runs/35963047330)) — see `docs/ACCEPTANCE_REPORT.md`.
